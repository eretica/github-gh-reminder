...
export function SettingsForm({ settings, onSave, loading }: SettingsFormProps): JSX.Element {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        ...
        <label className="flex items-start gap-3">
          <input
            ...
            onChange={(e) => setFormData({ ...formData, notifyOnNew: e.target.checked })}
            className="mt-1 h-4 w-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
          />
          <div>
            <span className="text-sm text-gray-900">Notify when new PR is assigned</span>
        ...
        <label className="flex items-start gap-3">
          ...
          />
          <div>
            <span className="text-sm text-gray-900">Enable periodic reminders</span>
            <p className="text-xs text-gray-500 mt-0.5">
        ...
        {formData.enableReminder && (
          <div className="ml-7">
            <label className="block text-sm text-gray-700 mb-1">Reminder interval</label>
            <select
              value={formData.reminderIntervalHours}
              onChange={(e) =>
                setFormData({ ...formData, reminderIntervalHours: Number(e.target.value) })
              }
      ...
      <div className="pt-4 border-t border-gray-200">
        ...
        {hasChanges && !saving && (
          <span className="ml-3 text-xs text-gray-500">You have unsaved changes</span>
        )}
      </div>
    </div>
  )
