...
export function PullRequestList({ pullRequests, repositories, onOpenPR }: PullRequestListProps): JSX.Element {
  ...
  if (repositories.length === 0) {
    return (
      <div className="text-center py-8 text-gray-500">
        ...
        <p className="text-sm mt-1 mb-4">Add a repository to start tracking PRs.</p>
        <button
        ...
        return (
          <div key={repo.id}>
            ...
            <div className="space-y-2 ml-1">
              {repoPRs.length > 0 ? (
                repoPRs.map((pr) => (
                  <PullRequestItem key={pr.id} pullRequest={pr} onOpen={onOpenPR} />
                ))
              ) : (
