...
class PRScheduler {
  ...
  async checkAllRepositories(settings: Settings): Promise<PullRequest[]> {
    ...
    const repos = await db.select().from(schema.repositories).where(eq(schema.repositories.enabled, 1))
  ...
  private async checkRepository(
    ...
    for (const ghPR of ghPRs) {
      ...
      if (existing) {
        ...
        const pr = this.toFrontendPR(newPR as schema.PullRequestRecord, repo.name)
    ...
    for (const [, pr] of existingPRMap) {
      await db.delete(schema.pullRequests).where(eq(schema.pullRequests.id, pr.id))
    }
  ...
  private async sendReminders(settings: Settings): Promise<void> {
    ...
    const db = getDatabase()
    const now = new Date()
    const reminderThreshold = new Date(now.getTime() - settings.reminderIntervalHours * 60 * 60 * 1000)
    ...
    const prsToRemind = await db
      ...
      .from(schema.pullRequests)
      .innerJoin(schema.repositories, eq(schema.pullRequests.repositoryId, schema.repositories.id))
      .where(
        and(
          eq(schema.repositories.enabled, 1),
          lt(schema.pullRequests.lastRemindedAt, reminderThreshold.toISOString())
        )
      )
    ...
    // Remove duplicates
    const uniquePRs = Array.from(new Map(allPRsToRemind.map((item) => [item.pr.id, item])).values())

    if (uniquePRs.length === 0) return

    const frontendPRs = uniquePRs.map((item) => this.toFrontendPR(item.pr, item.repo.name))
