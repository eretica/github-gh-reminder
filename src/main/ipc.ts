...
import { IPC_CHANNELS, DEFAULT_SETTINGS, type Settings, type Repository, type PullRequest } from '../shared/types'
import { scheduler } from './scheduler'
import { updateTrayMenu } from './tray'
import { createSettingsWindow } from './windows'
import { isGitRepository, getRepoName } from './gh-cli'
...
export function setupIpcHandlers(): void {
  ...
  ipcMain.handle(IPC_CHANNELS.REPO_LIST, async (): Promise<Repository[]> => {
    ...
    const repos = await db.select().from(schema.repositories).orderBy(schema.repositories.order)
    return repos.map((r) => ({
      id: r.id,
      path: r.path,
      name: r.name,
  ...
  ipcMain.handle(IPC_CHANNELS.REPO_ADD, async (event): Promise<Repository | null> => {
    ...
    const maxOrder = maxOrderResult.reduce((max, r) => Math.max(max, r.order), -1)
  ...
  ipcMain.handle(IPC_CHANNELS.REPO_REMOVE, async (_, id: string): Promise<void> => {
    const db = getDatabase()
  ...
  ipcMain.handle(IPC_CHANNELS.REPO_TOGGLE, async (_, id: string, enabled: boolean): Promise<void> => {
    const db = getDatabase()
    const now = new Date().toISOString()
    await db
  ...
  ipcMain.handle(IPC_CHANNELS.REPO_REORDER, async (_, ids: string[]): Promise<void> => {
    const db = getDatabase()
    const now = new Date().toISOString()
  ...
  ipcMain.handle(IPC_CHANNELS.SETTINGS_SET, async (_, newSettings: Settings): Promise<void> => {
    const db = getDatabase()
  ...
  ipcMain.handle(IPC_CHANNELS.PR_OPEN, async (_, url: string): Promise<void> => {
